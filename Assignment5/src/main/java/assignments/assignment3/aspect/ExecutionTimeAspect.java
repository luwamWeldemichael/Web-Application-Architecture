...
public class ExecutionTimeAspect {
    ...
    public Object calculateExecutionTime(ProceedingJoinPoint proceedingJoinPoint){
        ...
        try{
            ...
            Logger logger = new Logger((long)id[0], LocalDate.now(), (int)(finish - start), User.principal, proceedingJoinPoint.getSignature().getName());
            loggerRepo.save(logger);
        }
        catch(Throwable t){
            Object[] id = proceedingJoinPoint.getArgs();
            long finish = System.nanoTime();
            MyException exception = new MyException((long)id[0],LocalDate.now(),(int)(finish-start),User.principal, proceedingJoinPoint.getSignature().getName(), t.getClass().getSimpleName());
